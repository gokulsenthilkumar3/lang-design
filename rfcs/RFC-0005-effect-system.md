...
| Async task switch latency | < 1 µs | Competitive with Tokio/Go runtime |
...
- Should effects be a row type (extensible, like Koka) or a fixed set of
  built-in capabilities? Row types are more expressive but harder to infer.
- How does the `Panic` effect interact with the `?` error propagation
  operator?
- Should handlers be stackable (multiple handlers for the same effect) or
  exclusive (one handler per effect per scope)?
- What is the zero-cost proof strategy for pure functions?
