...
> Design repository for a new systems programming language — memory safe by default, AI-native, gradual formalism, unified toolchain.
...
This is the design and specification repository for a programming language that targets the gap between Rust (correctness), Python (iteration speed), and Go (deployment simplicity) — and aims to make context-switching between all three unnecessary.

This repository contains:
- **RFCs** — every design decision is an RFC, discussed publicly
- **Research notes** — prior art, academic papers, benchmarks informing design decisions
- **Syntax sketches** — illustrative (not final) code examples
- **Governance documents** — how decisions get made

No compiler code lives here yet. The rule: **every line of compiler code must have a corresponding accepted RFC.**
...
See [RFC-0001](rfcs/RFC-0001-language-vision-and-non-goals.md) for the full vision and non-goals.
...
|----------|-------------|--------|
| **Memory safety** | Affine ownership + region inference. No GC. No unsafe by default. | RFC-0001 Draft |
| **Gradual formalism** | Refinement types, opt-in per function. Informal code is valid code. | RFC-0001 Draft |
| **Intent-aware compiler** | Type system encodes purpose, not just shape. Errors explain semantics. | RFC-0001 Draft |
| **Unified toolchain** | Compiler + build + package manager + formatter + LSP = one binary. | RFC-0001 Draft |
| **AI-native types** | Tensor shape, graph structure in stdlib. GPU backend first-class. | RFC-0001 Draft |
| **WASM primary target** | WASM is a first-class compilation target, not a cross-compile afterthought. | RFC-0001 Draft |
...
├── .github/
│   ├── ISSUE_TEMPLATE/      # Issue templates: RFC idea, discussion, research
...
1. **Open an issue** using the [New RFC Idea](.github/ISSUE_TEMPLATE/new_rfc_idea.md) template.
...
| **5 — Ecosystem & Community** | Package registry, docs site, first external contributors | 2028 |
...
All contributions are welcome — research findings, RFC feedback, syntax critique, or entirely new RFC proposals. Read [CONTRIBUTING.md](CONTRIBUTING.md) to get started.
...
Building this as part of a longer-term platform vision covering both a language and an OS.  
